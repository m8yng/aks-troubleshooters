...
for c in ps.get("containers",[]):
    ...
    key=None
    if img_ref and img_ref in img_by_id: key=("id",img_ref)
    elif img_name and img_name in img_by_name: key=("name",img_name)
    if key:
        pod_images[uid].add(key)
        image_pods.setdefault(key,set()).add(uid)
...
for uid,keys in pod_images.items():
    ...
    for kind,val in keys:
        s+= (img_by_id[val] if kind=="id" else img_by_name.get(val,0))
    images_raw[uid]=s
...
for uid,keys in pod_images.items():
    s=0.0
    for key in keys:
        sz = img_by_id[key[1]] if key[0]=="id" else img_by_name.get(key[1],0)
        n = max(1, len(image_pods.get(key,[])))
        s += sz / n
    images_share[uid]=int(s)

rows=[]
for uid,name in pods_name.items():
    w=writable.get(uid,0); e=emptydir.get(uid,0); l=logs.get(uid,0)
    im_raw=images_raw.get(uid,0); im_share=images_share.get(uid,0)
    t=w+e+l+im_share
    if t>0 or im_raw>0: rows.append((t,w,e,l,im_raw,im_share,name,uid))
rows.sort(key=lambda r:r[0], reverse=True)

print("=== container ===")
print(f"{'Total':>9}  {'Writable':>9}  {'emptyDir':>9}  {'Logs':>9}  {'ImgRaw':>9}  {'ImgShare':>9}  Pod")
for t,w,e,l,imr,ims,name,uid in rows[:TOP]:
    print(f"{human(t):>9}  {human(w):>9}  {human(e):>9}  {human(l):>9}  {human(imr):>9}  {human(ims):>9}  {name} ({uid})")
